...
down=0;   %1=yes; 0=no (this assumes the data is available in _ICTP_SPEEDY.mat format; *not* NetCDF!)
...
%Xp=double(ncread(iridl,'lon'));
%Yp=double(ncread(iridl,'lat'));


pr1 = double(ncread(iridl,'prec'));
...
if down==1
  ...
  phi = double(ncread(iridl,'gh_500'));
  else
...
int_dP = double(ncread(iridl,'aprod'));
...
set(gca,'FontSize',14)
xlabel('WT'); ylabel('Niño3.4 Correlation' )
axis([0 nclust+1 -.5 .5])
set(gca,'YTick',[-.5:.1:.5])
